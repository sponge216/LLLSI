...
namespace concurrency {
	...
	concurrency::ThreadManager::~ThreadManager() {
		auto dwThreadArraySize = this->threadVector.size();
		auto dwStopEventVectorSize = this->ghStopEventVector.size();

		for (int i = 0; i < dwStopEventVectorSize; i++) {
			SetEvent(this->ghStopEventVector[i]); // signal event
		}
		...
		for (int i = 0; i < this->ghStopEventVector.size(); i++) {
			CloseHandle(this->ghStopEventVector[i]); // close event
		}
	...
	bool concurrency::ThreadManager::createNewThread(ConThread ctThread) {
		...
		if (!this->addNewStopEvent()) {
			...
			// add more safety measures, kill thread and alert user.
		}
		return true;
	}
	bool concurrency::ThreadManager::addNewStopEvent() {
		this->ghStopEventVector.push_back(CreateEvent(NULL, TRUE, FALSE, NULL));
		return true;
	}
